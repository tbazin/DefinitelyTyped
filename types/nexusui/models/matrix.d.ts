...
export type MatrixPattern = MatrixValue[][];
...
export default class Matrix {
    ...
    iterate(f: (row: number) => void, f2: (row: number, column: number, index: number) => void): void;
    formatAsText(): string;
    log(): void;
    update(pattern: MatrixPattern): void;
    get length(): number;
    locate(index: number): { row: number; column: number };
